...
mindmaps.OpenDocumentPresenter = function(eventBus, mindmapModel, view, filePicker) {
  ...
  view.openCloudButtonClicked = function(e) {
	 ...
	 * valid mindmaps Document. As adobe air doesn't support filereader API
	 * currently using the sandbox bridge method
	 * 
	 * @ignore
	 */
	view.openExernalFileClicked = function(e) {
		...
		var file = files[0];
		alert(file.fileName);
		var ideaFileContents = window.parentSandboxBridge.getFileContents(file.fileName);

		try {
			var doc = mindmaps.Document.fromJSON(ideaFileContents);
		} catch (e) {
